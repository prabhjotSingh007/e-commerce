...
export class CartService {
  ...
  AddProductToCart(product: any) {
    ...
    if (productExists) {
      ...
      const newProduct = { ...product, quantity: 1, totalPrice: product.price, oldPrice: product.price };
      const updatedCart = [...currentCart, newProduct];
      this.addedToCartProductSubject.next(updatedCart);

      console.log(this.addedToCartProductSubject.value)
  ...
  IncreaseProductQuantity(productId: number) {
    ...
    const updatedCart = currentCart.map((item) => {
      if (item.id === productId) {
        ...
        const updatedTotalPrice = updatedQuantity * item.oldPrice; // Update total price based on quantity
        console.log(item.price)
    ...
    const updatedCart = currentCart.map((item) => {
      if (item.id === productId) {
        ...
        if (item.quantity > 1) {
          ...
          console.log(item.price)
          const updatedTotalPrice = updatedQuantity * item.oldPrice; // Update total price based on new quantity
          console.log(item.price)
